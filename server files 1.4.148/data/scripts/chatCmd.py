...
class chatOptions(object):
    ...
    def checkAdmin(self,nick): # check host (settings.cmdForMe)
        ...
        if (client_str in gph.adminHashes) or (client_str in gph.royalpass) :
                                    ...
                                    try:
                                        if (i.getName()).encode('utf-8') == (a[0]):
                                            ...
                                            if bannedPlayerID not in foolist:
                                              ...
                                              bsInternal._chatMessage(str(bannedName) + " mixed")
                                              i.removeFromGame()
                                            else:
                                              bsInternal._chatMessage(str(bannedName) + " Already mixed")
                                            with open(bs.getEnvironment()['systemScriptsDirectory'] + "/getPermissionsHashes.py") as file:
                                                s = [row for row in file]
                        ...
                        if a == []:
                            for i in range(len(activity.players)):
                                if activity.players[i].getName().encode('utf-8').find(nick.encode('utf-8').replace('...','').replace(':','')) != -1:
                                    ...
                                    activity.players[i].actor.node.invincible = activity.players[i].actor.node.invincible == True
                    ...
                    if a == []:
                        ...
                        try:
                            if a[0] == 'all':
                                ...
                                if a[1]=='bunny' or a[1]=='ali' or a[1]=='cyborg' or a[1]=='agent':
                                    n = int(a[0])
